...
class ObjectDetector(Module):
    class Result:
        ...
        def __init__(self, output, class_labels):
            self.image_id = output[0]
            self.label = class_labels[int(output[1])]
    ...
    def __init__(self, model, class_labels, confidence_threshold=0.5, roi_scale_factor=1.15):
        super(ObjectDetector, self).__init__(model)
    ...
    def enqueue(self, input):
        return super(ObjectDetector, self).enqueue({self.input_blob: input})
    ...
    def get_roi_proposals(self, frame):
        ...
        for output in outputs[0][0]:
            result = ObjectDetector.Result(output, self.class_labels)
